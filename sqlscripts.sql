...
select count(1) from otodom_data;
select * from otodom_data limit 10;
...
select * from otodom_data_log

select * from otodom_data_flatten_translate;

create or replace table OTODOM_DATA_TRANSFORMED as

with cte as(

    SELECT ot.*,
        TRIM(SPLIT_PART(location, ',', -2)) AS suburb,
        TRIM(SPLIT_PART(location, ',', -3)) AS city,
        TRIM(SPLIT_PART(location, ',', -1)) AS region
    FROM otodom_data_flatten ot
    left join otodom_data_flatten_translate trans on ot.rn=trans.rn
    WHERE location LIKE 'ul.%')
select *
, case when lower('title_eng') like '%commercial%' or lower('title_eng') like '%office%' or lower('title_eng') like '%shop%' then 'non apartment'
       when is_for_sale = 'false' and surface <=330 and price <=55000 then 'apartment'
       when is_for_sale = 'false' then 'non apartment'
       when is_for_sale = 'true'  and surface <=600 and price <=20000000 then 'apartment'
...
1. What is the average rental price of 1 room, 2 room, 3 room and 4 room apartments
in some of the major SUBURB in Poland?
Arrange the result such that avg rent for each type fo room is shown in seperate
column
...
from (
select SUBURB,no_of_rooms,price
from otodom_data_transformed
where city in ('kołobrzeski', 'Siemianowice Śląskie', 'Warszawa', 'Chorzów', 'Piotrków Trybunalski', 'Kraków','Rzeszów')
...
2. I want to buy an apartment which is around 90-100 m2 and within a range of
800,000 to 1M, display the suburbs in warsaw where I can find such apartments.
...
3.What size of an apartment can I expect with a monthly rent of 3000 to 4000 PLN in
different major cities of Poland?
...
4)What are the most expensive apartments in major cities of Poland? Display the ad
title in english along with city, suburb, cost, size.
...
6. What is the avg sale price for apartments within 50-70 m2 area in major cities of
Poland?
...
Otodom Data Analysis 4
from cte2
group by suburb
order by suburb;


8. Which are the top 3 most luxurious neighborhoods in Warsaw? Luxurious
neighborhoods can be defined as suburbs which has the most no of of apartments
costing over 2M in cost.
...
9. Most small families would be looking for apartment with 40-60 m2 in size. Identify
the top 5 most affordable neighborhoods in warsaw.
...
11. What is the average rental price and sale price in some of the major cities in
Poland?
with cte as
(select city
